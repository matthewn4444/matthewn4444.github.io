var SubtitlesOctopus = function (options) {
    ...
    console.log(getFormattedDate(), "renderFrames", self.canvas.width, self.canvas.height)
        var data = self.renderFramesData;
        var beforeDrawTime = performance.now();
    console.log(getFormattedDate(), "before clear", data.canvases.length)
        self.ctx.clearRect(0, 0, self.canvas.width, self.canvas.height);
    console.log(getFormattedDate(), "after clear", data.canvases.length)
        for (var i = 0; i < data.canvases.length; i++) {
            var beforeTime = performance.now();
            var image = data.canvases[i];
    console.log(getFormattedDate(), "       inside loop render #" + i, image.w, image.h)
            self.bufferCanvas.width = image.w;
            self.bufferCanvas.height = image.h;
    console.log(getFormattedDate(), "       inside loop render (2), make arrya")
            
            var imageBuffer = new Uint8ClampedArray(image.buffer);
    console.log(getFormattedDate(), "       inside loop render (3), make image byffer")
            var imageData = new ImageData(imageBuffer, image.w, image.h);
    console.log(getFormattedDate(), "       inside loop render (4), put image data")
            self.bufferCanvasCtx.putImageData(imageData, 0, 0);
    console.log(getFormattedDate(), "       inside loop render (5), draw")
            self.ctx.drawImage(self.bufferCanvas, image.x, image.y);
            console.log(getFormattedDate(), '      Single frame draw (#' + i + '): ' + (Math.round(performance.now() - beforeTime)) + ' ms');
        }
        if (self.debug) {
            var drawTime = Math.round(performance.now() - beforeDrawTime);
            console.log(getFormattedDate(), Math.round(data.spentTime) + ' ms (+ ' + drawTime + ' ms draw)');
                ...
                switch (data.op) {
                    ...
                    case 'render': {
                        ...
                        if (!self.renderFrameData) {
                            console.log(getFormattedDate(), "render");
                    ...
                    case 'renderMultiple': {
                        if (self.lastRenderTime < data.time) {
                            console.log(getFormattedDate(), "    renderMultiple", self.lastRenderTime, data.time, data.canvases.length);
                            self.lastRenderTime = data.time;
                            self.renderFramesData = data;
                            console.log(getFormattedDate(), "    requestAnimationFrame");
    ...
    self.addDialogEvent = function (content) {
        self.worker.postMessage({
            target: 'add-dialog-event',
            content: content
        });
    };
